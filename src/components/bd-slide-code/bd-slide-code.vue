    ...
    >
      ...
      >
        ...
        <canvas
          ...
          :height="calcHeight"
          ref="canvas2"
          :style="`width:70px;height:${calcHeight}px;transform:translateX(${styleWidth - 50 - 20*((styleWidth - 50)/(calcWidth-50))}px)`"
        />
        <div :class="['loading-box',{'hide': !loading}]">
          <span class="el-icon-loading loading-icon" />
        </div>
        <div :class="['info-box',{'show':infoBoxShow},{'fail':infoBoxFail}]">
          {{ infoText }}
        </div>
        <div
          :class="['flash',{'show': isSuccess}]"
          :style="`transform: translateX(${isSuccess ? `${calcWidth + 100}px` : '-50px'}) skew(-30deg, 0);`"
        />
      </div>
      <span
        title="刷新验证码"
        @click="refresh"
        class="el-icon-refresh-left refresh-icon"
      />
...
export default {
  ...
  props: {
    ...
    pointX: { type: Number, default: 150 }, // 主canvas的宽
    pointY: { type: Number, default: 80 }, // 主canvas的高
    // 是否出现，由父级控制
  ...
  computed: {
    styleWidth() {
      ...
      return w < 50 ? 50 : w > this.calcWidth ? this.calcWidth : w;
    },
    calcWidth() {
      return this.canvasWidth < 310 ? 310 : this.canvasWidth;
    },
    calcHeight() {
      return this.canvasHeight < 160 ? 160 : this.canvasHeight;
    }
  },
  ...
  methods: {
    ...
    init(withCanvas) {
      ...
      ctx2.clearRect(0, 0, 70, this.calcHeight);

      // 取一个随机坐标，作为拼图块的位置
      // this.pinX = this.getRandom(90, this.calcWidth - 90); // 留20的边距
      // this.pinY = this.getRandom(20, this.calcHeight - 90);
      this.pinX = this.pointX;
      this.pinY = this.pointY;
      ...
      img.onload = () => {
        ...
        ctx.fill();
        ctx.restore();
        ctx.drawImage(img, 0, 0, this.calcWidth, this.calcHeight);
        ctx3.drawImage(img, 0, 0, this.calcWidth, this.calcHeight);
    ...
    makeImgWithCanvas() {
      ...
      ctx.fillRect(0, 0, this.calcWidth, this.calcHeight);
      // 随机画10个图形
      for (let i = 0; i < 10; i++) {
        ctx.fillStyle = `rgb(${this.getRandom(100, 255)},${this.getRandom(
          100,
        ...
        if (this.getRandom(0, 2) > 1) {
          ...
          ctx.fillRect(
            this.getRandom(-20, this.calcWidth - 20),
            this.getRandom(-20, this.calcHeight - 20),
            this.getRandom(10, this.calcWidth / 2 + 10),
            this.getRandom(10, this.calcHeight / 2 + 10)
          );
          ctx.restore();
        } else {
          // 圆
          ...
          ctx.arc(
            this.getRandom(0, this.calcWidth),
            this.getRandom(0, this.calcHeight),
            this.getRandom(10, this.calcHeight / 2 + 10),
            ran,
            ran + Math.PI * 1.5
          );
    ...
    // 开始判定
    submit() {
      let drag = this.styleWidth - 50 - 20 * ((this.styleWidth - 50) / (this.calcWidth - 50)) + 3;
      drag = drag.toFixed(2);
      ...
      const x = Math.abs(
        this.pinX -
          (this.styleWidth - 50) +
          20 * ((this.styleWidth - 50) / (this.calcWidth - 50)) -
          3
      );
      if (x < 5) {
        // 成功
